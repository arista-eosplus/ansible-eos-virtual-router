...
/*
 * Jenkinsfile for ansible-eos-virtual-router role
 *
 * Run the Ansible-Role-Test job against a commit or
 * pull request in the ansible-eos-virtual-router repo
 */
...
pipeline {
    ...
    environment {
        projectName = 'ansible-eos-virtual-router'
        emailTo = 'ansible-dev+jenkins@arista.com'
        emailFrom = 'eosplus-dev@arista.com'
    }

    stages {
        stage ('Run tests for ansible-eos-virtual-router role') {
            when {
            ...
            steps {
                ...
                build job: 'Ansible-Role-Test-Starter',
                      parameters: [
                          string(name: 'ROLE_NAME', value: 'ansible-eos-virtual-router'),
